from contraqctor.qc import Suite, ContextExportableObj
...
class SingleSiteBehaviorSuite(Suite):
    ...
    def test_metrics(self):
        return self.pass_test(self.session_dataset.session_metrics)

    def test_visualize_raw_ethogram(self):
        try:
            fig, ax = plt.subplots(figsize=(12, 6))
            ax_velocity, ax_events = plot_ethogram(
                self.session_dataset.dataset,
    ...
    def test_visualize_summary_behavior(self):
        try: 
            ...
            blocks = (
                self.session_dataset.dataset["Behavior"]["SoftwareEvents"]["Block"].load().data.copy()
            )
